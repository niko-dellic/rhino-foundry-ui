...
/// <summary>
/// Responsive image gallery. Item images are borrowed; callers retain disposal ownership. The
...
/// </summary>
public class FoundryThumbnailGallery : Drawable
{
    ...
    private FoundryThumbnailItem[] _items = [];
    ...
    public FoundryThumbnailGallery()
        : base(true)
    {
    ...
    public event EventHandler<FoundryThumbnailSelectionEventArgs>? SelectionChanged;

    public float ContentHeight => _contentHeight;

    public void SetItems(IEnumerable<FoundryThumbnailItem> items)
    {
    ...
    public void SetLayout(double availableWidth, int requestedTileWidth, int minimumHeight = 120)
    {
    ...
    private void DrawItem(Graphics graphics, int index, RectangleF bounds, FoundryThumbnailItem item)
    {
    ...
    private void OnMouseMove(object? sender, MouseEventArgs eventArgs)
    {
        ...
        if (distance <= 6 || string.IsNullOrEmpty(DragDataFormat)) return;
        var data = new DataObject();
        data.SetString(_pressedName, DragDataFormat);
    ...
    private void Select(string? name)
    {
        if (string.Equals(_selectedName, name, StringComparison.OrdinalIgnoreCase)) return;
        _selectedName = name;
        SelectionChanged?.Invoke(this, new FoundryThumbnailSelectionEventArgs(name));
        Invalidate();
    }
...
public sealed record FoundryThumbnailItem(string Name, Image? Image);

public sealed class FoundryThumbnailSelectionEventArgs(string? name) : EventArgs
{
