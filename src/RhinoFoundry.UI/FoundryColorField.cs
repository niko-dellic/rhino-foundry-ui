...
/// </summary>
public sealed class FoundryColorField : Drawable
{
    ...
    public FoundryColorField(
        Color value,
        int width = 170,
        string toolTip = "Choose a color",
        bool allowAlpha = false)
        : base(true)
    {
        _value = Normalize(value, allowAlpha);
    ...
    public Color Value
    {
        ...
        set
        {
            var normalized = Normalize(value, _dialog.AllowAlpha);
            if (SameColor(_value, normalized, _dialog.AllowAlpha)) return;
            _value = normalized;
    ...
    private void OnPaint(object? sender, PaintEventArgs eventArgs)
    {
        ...
        var valueText = _dialog.AllowAlpha
            ? $"#{_value.Rb:X2}{_value.Gb:X2}{_value.Bb:X2}{_value.Ab:X2}"
            : $"#{_value.Rb:X2}{_value.Gb:X2}{_value.Bb:X2}";
    ...
    private static bool SameColor(Color left, Color right, bool includeAlpha) =>
        left.Rb == right.Rb &&
        left.Gb == right.Gb &&
        left.Bb == right.Bb &&
        (!includeAlpha || left.Ab == right.Ab);

    private static Color Normalize(Color color, bool allowAlpha) =>
        allowAlpha ? color : Color.FromArgb(color.Rb, color.Gb, color.Bb, 255);
}
